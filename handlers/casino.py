...
def register_casino_handlers(dp):
    ...
    async def casino_bet(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            await state.set_state("waiting_for_casino_bet")
        except Exception as e:
    ...
    async def process_casino_bet(message: types.Message, state: FSMContext):
        """Обработчик ставок в казино (в user.py) - дублируем на всякий случай"""
    ...
    async def slots_spin(callback: types.CallbackQuery):
        ...
        try:
            ...
            # Новая логика слотов:
            # 1. 5% шанс на "777" → x3
            # 2. 35% шанс на 2 одинаковых → x2
            # 3. Остальное - проигрыш
            
            if random.random() < 0.05:  # 5% - Джекпот "777"
                if result[0] == "7️⃣" and result[1] == "7️⃣" and result[2] == "7️⃣":
                    win = bet * 3
                    win_text = "🎰 ДЖЕКПОТ! 7️⃣7️⃣7️⃣"
                else:
                    # Если не выпало 777, но сработал шанс - перекидываем
                    result = [random.choice(symbols) for _ in range(3)]
                    if result[0] == result[1] or result[1] == result[2] or result[0] == result[2]:
                        win = bet * 2
                        win_text = "🎰 ДВЕ ОДИНАКОВЫХ!"
            
            elif random.random() < 0.35:  # 35% - 2 одинаковых
                if result[0] == result[1] or result[1] == result[2] or result[0] == result[2]:
                    win = bet * 2
                    win_text = "🎰 ДВЕ ОДИНАКОВЫХ!"
            
            # Если выпало 3 одинаковых (кроме 777) - дополнительный бонус
    ...
    async def mines_settings(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            await state.set_state("waiting_for_mines_count")
        except Exception as e:
    ...
    async def mines_field(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            await state.set_state("waiting_for_field_size")
        except Exception as e:
