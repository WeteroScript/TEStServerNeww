...
from services.auction import refresh_auction_for_all, set_admin_auction_lots_with_slot
...
def register_admin_handlers(dp):
    ...
    async def car_list(message: types.Message):
        ...
        try:
            ...
            for i, name in enumerate(car_list, 1):
                ...
                stars = get_stars_display(data['stars'])
                text += f"**{i}. {name}**\n"
                text += f"   🆔 ID: `car_{i}`\n"
    ...
    async def get_player_acc(message: types.Message):
        ...
        try:
            ...
            keyboard = [
                ...
                [InlineKeyboardButton(text="🔒 Обнулить замороженный", callback_data=f"admin_reset_frozen_{found_id}")],
    ...
    @dp.callback_query(F.data.startswith("admin_reset_frozen_"))
    async def admin_reset_frozen(callback: types.CallbackQuery):
        if not await is_admin(callback.from_user.id):
            await callback.answer("⛔ Нет прав!", show_alert=True)
            return
        
        user_id = callback.data.replace("admin_reset_frozen_", "")
        try:
            users = await load_users()
            if user_id in users:
                users[user_id]["frozen_balance"] = 0
                await save_users(users)
                await callback.answer("✅ Замороженный баланс обнулен!", show_alert=True)
        except Exception as e:
            logger.error(f"Ошибка: {e}")
            await callback.answer("❌ Ошибка!", show_alert=True)
        await callback.answer()
        ...
        try:
            ...
            for user_id, data in users.items():
                try:
                    user = await bot.get_chat(int(user_id))
                    if user.username and user.username.lower() == username:
                        user_business_count = sum(1 for biz in data.get("business", {}).values() if biz.get("owned", False))
