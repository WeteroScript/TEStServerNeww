...
def register_user_handlers(dp):
        ...
        try:
            ...
            for i, (user_id, data) in enumerate(sorted_users, 1):
                try:
                    ...
                    logger.warning(f"Не удалось получить юзера {user_id}: {e}")
    ...
    async def promo_menu(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            await state.set_state("waiting_for_promo")
        except Exception as e:
    ...
    async def handle_all_messages(message: types.Message, state: FSMContext):
        ...
        if current_state == "waiting_for_trade_amount":
            try:
                ...
                logger.info(f"Трейдинг: currency={currency}, action={action}, price={price}")
            ...
            try:
                ...
                for admin_id in ADMIN_IDS:
                    try:
                        ...
                        sent_to += 1
                        logger.info(f"✅ Сообщение поддержки отправлено админу {admin_id}")
                    except Exception as e:
        ...
        # ===== 7. ПРОМОКОДЫ =====
        # ==========================================
        if current_state == "waiting_for_promo":
            try:
                users = await load_users()
                ...
                if promo["type"] == "brcoins":
                    user["brcoins"] = user.get("brcoins", 0) + promo["amount"]
                    user["donate_received"] = user.get("donate_received", 0) + promo["amount"]
                    reward_text = f"+{promo['amount']} BRcoins"
                else:
                    user["money"] = user.get("money", 0) + promo["amount"]
                    user["total_earned"] = user.get("total_earned", 0) + promo["amount"]
                    reward_text = f"+{promo['amount']:,.0f}₽"
        ...
        # ===== 8. БИЗНЕС (продажа) =====
        # ==========================================
        if current_state == "waiting_for_business_sell":
            try:
                ...
                if not business_id or business_id not in user.get("business", {}):
                    await message.answer("❌ Бизнес не найден!")
                    await state.clear()
                    return
                
                business = user["business"][business_id]
                
                if quantity > business.get("count", 0):
                    await message.answer(f"❌ У вас только {business.get('count', 0)} этого бизнеса!")
                    return
                
                # 50% от стоимости
                price_per_unit = business.get("price", 0)
                sell_price = int(price_per_unit * 0.5 * quantity)
                
                business["count"] = business.get("count", 0) - quantity
                user["money"] += sell_price
                ...
                await message.answer(
                    f"✅ Продано {quantity} бизнеса!\n\n"
                    f"💰 Получено: {sell_price:,.0f}₽ (50%)\n"
                    f"💳 Новый баланс: {user['money']:,.0f}₽",
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    ...
    # ==========================================
    # ===== CALLBACK ДЛЯ БИЗНЕСА =====
    # ==========================================
    
    @dp.callback_query(F.data.startswith("business_sell_"))
    async def business_sell_callback(callback: types.CallbackQuery, state: FSMContext):
        """Прямая продажа бизнеса без диалога"""
        if not await check_access(callback):
            return
        
        try:
            business_id = callback.data.replace("business_sell_", "")
            user_id = str(callback.from_user.id)
            ...
            business = user["business"][business_id]
            quantity = business.get("count", 1)
            
            if quantity <= 0:
                await callback.answer("❌ Нет бизнесов для продажи!", show_alert=True)
                return
            
            # 50% от стоимости за все
            price_per_unit = business.get("price", 0)
            sell_price = int(price_per_unit * 0.5 * quantity)
            ...
            await callback.message.edit_text(
                f"✅ Продано {quantity} {business['name']}!\n\n"
                f"💰 Получено: {sell_price:,.0f}₽ (50%)\n"
                f"💳 Новый баланс: {user['money']:,.0f}₽",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="🔙 В бизнес", callback_data="business")]
