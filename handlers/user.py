...
def register_user_handlers(dp):
    ...
    async def handle_promo_and_support(message: types.Message, state: FSMContext):
        """
        Обрабатывает:
        1. Поддержку (состояние = waiting_for_support_message)
        2. Промокоды (НЕТ активного состояния)
        
        Пропускает:
        - Ставки на аукционе (waiting_for_auction_bid)
        - Трейдинг (waiting_for_trade_amount)
        - Казино (waiting_for_casino_bet, waiting_for_mines_count, waiting_for_field_size)
        """
        if not await check_access(message):
            return
        ...
        # ==========================================
        # ===== 2. ПРОПУСКАЕМ ВСЕ ОСТАЛЬНЫЕ СОСТОЯНИЯ =====
        # ==========================================
        # Если есть ЛЮБОЕ другое состояние - пропускаем!
        # Это позволяет auction.py, casino.py, jobs.py обрабатывать свои сообщения
        ...
        # ==========================================
        # Только если НЕТ состояния (current_state is None)
        try:
            user_id = str(message.from_user.id)
            users = await load_users()
